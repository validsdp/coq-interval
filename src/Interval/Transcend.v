...
Definition c1_p_c1_8 := F.add_DN (F.PtoP 52) c1 c1_8.  (* TODO: eval *compute in ? *)
...
{ unfold c1; rewrite F.one_correct; simpl; apply f_equal; lra. }
unfold F.toR, c1; rewrite F.one_correct; simpl; rewrite Rabs_R1; lra.
Qed.
...
Lemma i1_correct : contains (I.convert i1) (Xreal 1).
Proof.
simpl; unfold c1.
rewrite F'.valid_lb_real; [|now rewrite F.real_correct, F.fromZ_correct].
rewrite F'.valid_ub_real; [|now rewrite F.real_correct, F.fromZ_correct].
simpl.
rewrite F.fromZ_correct; [lra|lia].
Qed.
...
assert (H3le256 : (Z.abs 3 <= 256)%Z); [now compute|].
generalize i1_correct (I.fromZ_small_correct 3 H3le256).
fold i1 i3.
generalize i1 i3.
intros powi divi.
...
now apply I.fromZ_small_correct; compute.
...
intros thre n.
replace 1%R with (Ai x 0) by (unfold Ai ; simpl ; field).
assert (H2le256 : (2 <= 256)%Z); [now compute|].
generalize i1_correct (I.fromZ_small_correct 2 H2le256).
fold i1 i2.
generalize i1 i2.
intros powi divi.
...
{ intros prec xi x Hx Hxu Ix.
  ...
  { split. lra.
    ...
    elim (F.add_DN_correct (F.PtoP 52) F.one c1_8).
    2: now rewrite F'.valid_lb_one.
    2: now rewrite F'.valid_lb_real; [|rewrite F.real_correct, c1_8_correct].
    intros Va.
    case (F.toX _); [easy|intro r].
    rewrite c1_8_correct, F.one_correct.
    simpl.
    intro Hr.
...
{ (* x real *)
  ...
  rewrite F.real_correct, F.one_correct.
...
assert (H2le256 : (Z.abs 2 <= 256)%Z); [now compute|].
assert (H3le256 : (Z.abs 2 <= 256)%Z); [now compute|].
generalize i1_correct (I.fromZ_small_correct 2 H2le256) (I.fromZ_small_correct 3 H3le256).
fold i1 i2 i3.
generalize i1 i2 i3.
...
{ apply J.sub_correct.
  ...
  assert (H6le256 : (Z.abs 6 <= 256)%Z); [now compute|].
  assert (H4le256 : (Z.abs 4 <= 256)%Z); [now compute|].
  generalize i1_correct (I.fromZ_small_correct 6 H6le256) (I.fromZ_small_correct 4 H4le256).
  fold i1 i6 i4.
  generalize i1 i6 i4.
  intros powi divi facti.
  change 1%R with (pow (toR x) (2 * 0)).
...
{ simpl.
  unfold c1, cm1.
  rewrite F'.neg_correct, F'.valid_lb_neg, F.one_correct, F'.valid_ub_one.
  apply SIN_bound. }
...
- generalize (F.incr_prec prec (Z2P (F.StoZ (F.mag x) + 7))).
  ...
  { replace (Xabs (Xdiv (Xreal (sin (toR x))) (Xreal (cos (toR x)))))
    ...
    { apply I.sqrt_correct.
      apply I.sub_correct; [|easy].
      apply I.div_correct; [easy|].
      now apply I.sqr_correct. }
    unfold Xdiv'.
    simpl.
  ...
  unfold Xtan', Xbind.
  destruct s; [easy| |] ; case I.sign_large; try easy ; intros Hc'.
  { revert H.
  ...
  { rewrite <- (Xneg_involutive (if is_zero _ then _ else _)).
    ...
    apply Rle_antisym with (2 := H0).
    now specialize (Hc' _ Hc). }
  destruct (is_zero_spec (cos (toR x))) as [H0|H0]; [easy|].
  evar_last.
  apply H.
  apply (f_equal Xreal).
  apply Rabs_pos_eq.
...
{ (* zero *)
  ...
  case is_zero_spec.
  { rewrite cos_0.
    now intro H'; exfalso; apply R1_neq_R0. }
  intros _.
  rewrite tan_0.
...
assert (H2le256 : (Z.abs 2 <= 256)%Z); [now compute|].
assert (H3le256 : (Z.abs 3 <= 256)%Z); [now compute|].
generalize Ix (I.fromZ_small_correct 2 H2le256) (I.fromZ_small_correct 3 H3le256).
fold i2 i3.
generalize i2 i3.
...
{ (* nb = 0 *)
  ...
  simpl.
  unfold c1.
  rewrite F'.valid_lb_zero, F'.valid_ub_one, F.zero_correct, F.one_correct.
  split.
  { apply Rlt_le.
    apply exp_pos. }
  simpl.
...
(* pos too big *)
rewrite I.bnd_correct.
2: now apply I.valid_lb_real; unfold c1; rewrite F.one_correct.
