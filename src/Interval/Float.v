...
Definition midpoint xi :=
  ...
  | Ibnd xl xu =>
    ...
    | Xgt, Xund => F.mul_UP (F.PtoP 52) xl (F.fromZ 2)
    | Xund, Xlt => F.mul_DN (F.PtoP 52) xu (F.fromZ 2)
...
Definition midpoint' xi :=
  ...
  | Ibnd xl xu =>
    ...
    | Xeq, Xund => fromZ 1%Z
    | Xund, Xeq => fromZ (-1)%Z
    | Xgt, Xund => let m := F.mul_UP (F.PtoP 52) xl (F.fromZ 2) in Ibnd m m
    | Xund, Xlt => let m := F.mul_DN (F.PtoP 52) xu (F.fromZ 2) in Ibnd m m
    | _, _ =>
...
Definition mul2 prec xi :=
  ...
  | Ibnd xl xu =>
    ...
    Ibnd (F.mul_DN prec xl two) (F.mul_UP prec xu two)
  | Inan => Inan
  end.
...
Definition inv prec xi :=
  ...
  | Ibnd xl xu =>
    ...
    | _ => let one := F.fromZ 1 in
      Ibnd (Fdivz_DN prec one xu) (Fdivz_UP prec one xl)
    end
  | _ => Inan
  end.
