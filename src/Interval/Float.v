...
Definition empty := Ibnd F.one F.zero.
...
{ now simpl; case (_ && _). }
simpl.
rewrite F.one_correct, F.zero_correct; case (_ && _); simpl; lra.
Qed.
...
Definition midpoint xi :=
  ...
  | Ibnd xl xu =>
    ...
    | Xund, Xgt => F.zero
    | Xeq, Xund => F.one
    | Xund, Xeq => F.neg F.one
    | Xgt, Xund => F.mul_UP (F.PtoP 52) xl (F.fromZ_UP 2)
    | Xund, Xlt => F.mul_DN (F.PtoP 52) xu (F.fromZ_UP 2)
...
Definition midpoint' xi :=
  ...
  | Ibnd xl xu =>
    ...
    | Xlt, Xund => zero
    | Xund, Xgt => zero
    | Xeq, Xund => Ibnd F.one F.one
    | Xund, Xeq => Ibnd (F.neg F.one) (F.neg F.one)
    | Xgt, Xund => let m := F.mul_UP (F.PtoP 52) xl (F.fromZ_UP 2) in Ibnd m m
    | Xund, Xlt => let m := F.mul_DN (F.PtoP 52) xu (F.fromZ_UP 2) in Ibnd m m
    | _, _ =>
...
Definition inv prec xi :=
  ...
  | Ibnd xl xu =>
    ...
    | Xeq => Inan
    | _ =>
      Ibnd (Fdivz_DN prec F.one xu) (Fdivz_UP prec F.one xl)
    end
  | _ => Inan
  end.
...
Definition power_int prec xi n :=
  ...
  | Z0 => match xi with Inan => Inan | _ => Ibnd F.one F.one end
...
{ admit.
  ...
  (*   now apply Rmult_le_compat_neg_l; [exact (Rlt_le _ _ H)|]. } *)
  (* { rewrite (proj1 (F.neg_correct _)), F.one_correct, H. *)
  (*   repeat split. *)
  (*   lra. } *)
  (* rewrite F.zero_correct. *)
  (* repeat split. *)
...
{ (* infinite upper *)
  ...
  (* apply Rmult_le_compat_l; [exact (Rlt_le _ _ H)|]. *)
  (* exact Hr. *) }
(* finite bounds *)
admit.
(* assert ( *)
...
Proof.
intro xi.
assert (H := midpoint'_valid_bounds xi); revert xi H.
intros [|xl xu] Hv.
{ split; [easy|].
  intros _.
  exists R0.
...
revert Hv; case_eq (midpoint' (Ibnd xl xu)); [now simpl|].
intros xl' xu' Hxl'xu' Hv.
unfold convert; rewrite Hv; clear Hv.
case (_ && _); [|now split].
revert Hxl'xu'.
unfold midpoint'.
Admitted.
...
(* unfold not_empty. *)
(* simpl. *)
(* assert (He: forall b, exists v : R, contains (Interval.Ibnd (F.toX b) (F.toX b)) (Xreal v)). *)
(* { intros b. *)
...
(*     rewrite F.zero_correct. *)
(*     split ; apply Rle_refl. } *)
(*   intro H; split; revert H. *)
(*   { case Rcompare_spec ; intros Hu0 H; *)
...
(*       apply Rmult_le_compat_neg_l; [now apply Rlt_le|]. *)
(*       now apply IZR_le, (Zpower_le _ 0 1). } *)
(*     { rewrite (proj1 (F.neg_correct _)), F.one_correct; split; [easy|]. *)
(*       revert H; intros [H1' H2']. *)
(*       rewrite (Rle_antisym _ _ H2' H1'), Hu0, <-opp_IZR. *)
(*       now apply IZR_le. } *)
(*     rewrite F.zero_correct. *)
(*     intros [H1' H2']. *)
(*     apply (conj I). *)
...
(*     now apply Rlt_le. } *)
(*   intros H _; revert H. *)
(*   case Rcompare; intro H; inversion H; apply He. } *)
(* case_eq (F.toX xu) ; [|intros xur] ; intros Hu ; simpl. *)
(* (* infinite upper *) *)
(* { intro H; split; revert H. *)
(*   { case Rcompare_spec; intros Hl0 H; inversion H; clear H; simpl; *)
(*       (intros [|x] ; [easy|]). *)
(*     { rewrite F.zero_correct. *)
(*       intros [H1' H2']. *)
(*       refine (conj _ I). *)
...
(* Qed. *)
(* ||||||| parent of c3119e4... Add valid_lb and valid_ub in Float.Sig to enable use of a float type *)
(* split. *)
(* intros x. *)
(* assert (contains (convert mi) x -> match x with Xnan => False | Xreal x => (xlr <= x <= xur)%R end). *)
(* unfold mi, m. clear -Hl Hu. *)
...
case (sign_strict_ xl xu) ;
  ...
  try rewrite F.one_correct ;
  try ( rewrite (F.valid_lb_correct F.one) ;
        [|now rewrite F.real_correct, F.one_correct] ) ;
  try ( rewrite (F.valid_ub_correct F.one) ;
        [|now rewrite F.real_correct, F.one_correct] ) ;
...
rewrite 2!F'.cmp_correct, F.zero_correct.
xreal_tac xl; xreal_tac xu; simpl.
{ now rewrite F.zero_correct; right. }
{ case Rcompare_spec; intro Hr; simpl;
  try ( match goal with
         |- context [F'.cmp ?x ?y] => case_eq (F'.cmp x y); intro Hcmp
        end );
  simpl;
  try (now rewrite F.zero_correct; right) ;
  revert Hcmp; rewrite F'.cmp_correct, F.zero_correct ;
  (xreal_tac2; [now simpl|]; simpl ;
   now case Rcompare_spec; [| |intros H _; left]). }
{ case Rcompare_spec; intro Hr; simpl;
  try ( match goal with
         |- context [F'.cmp ?x ?y] => case_eq (F'.cmp x y); intro Hcmp
        end );
  ...
  (xreal_tac2; [now simpl|]; simpl ;
   now case Rcompare_spec; [| |intros H _; left]). }
rewrite 2!F'.cmp_correct, F.zero_correct.
case Rcompare_spec; intro Hr; case Rcompare_spec; intro Hr0; simpl;
  try ( xreal_tac (F.mul_DN prec xu xu) ; simpl ) ;
  try ( xreal_tac (F.mul_DN prec xl xl) ; simpl ) ;
  try ( case Rcompare_spec ; intro Hr1 ; simpl ) ;
  try ( now rewrite F.zero_correct; right ) ;
  try (now rewrite X1 ; left ) ;
  try (now rewrite X2 ; left ).
Qed.
