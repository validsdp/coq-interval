...
Theorem sqr_ge_0 :
  forall prec xi, not_empty (convert xi) -> convert xi <> Interval.Inan ->
...
case_eq (F.classify xl) ; intros Cxl ; [..|easy] ;
  ...
  try ( case (Rcompare_spec rxu 0) ; intros Hrxu0 ) ;
  simpl ;
  try ( rewrite F.zero_correct ; right ; reflexivity ) ;
  rewrite F.cmp_correct, F'.classify_zero, ?F.zero_correct ;
  try match goal with
      | |- context [ F.mul_DN prec ?x _ ] =>
        elim (F.mul_DN_correct prec x x) ;
          [intros Vmdn Hmdn
          |rewrite ?Hrxl, ?Hrxu ; try ( left ; lra ) ; right ; left; lra] ;
          generalize (F.classify_correct (F.mul_DN prec x x)) ; rewrite F.real_correct ;
            generalize Vmdn ; rewrite F.valid_lb_correct ;
              case_eq (F.classify (F.mul_DN prec x x)); [..|easy] ; intros Cmdn _ ;
                [xreal_tac2; [easy|]|xreal_tac2; [|easy]..] ; intros _
      end ;
  simpl ;
  try ( rewrite F.zero_correct ; right ; reflexivity ) ;
